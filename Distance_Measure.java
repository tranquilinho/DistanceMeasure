...
import jgc.file.FileDialog;

/**
 * Basic workflow: see README
 * 
 * @author Jesus Cuenca
 * @depends: see requirements in README
 */

// TODO: auto threshold (improve initial values)
// TODO: print stderr logs only when Debug is enabled
...
public class Distance_Measure extends PlugInFrame {
	...
	private final static Color ROI_COLOR = Color.YELLOW;// new Color(128, 255,
														// 255);
	...
	private void addRegion(boolean fast) throws OutOfMemoryError,
		...
		if (imp != null) {
			...
			if (roi != null) {
				if (roi.isArea()) {
					...
					System.err.println("Red channel: " + redChannelNumber
							+ ". Green channel: " + greenChannelNumber);
					...
					double[] distanceInMicron = measure(green, red,
							cal.pixelHeight, fast);
					long finishTime = System.nanoTime();
					System.err.println("Elapsed time (measure): "
							+ (finishTime - startTime) / 1000000);
					startTime = System.nanoTime();

					Histogram h = new Histogram(distanceInMicron,
							HISTOGRAM_BINS, progressBar, 60, 20);
					h.fill(distanceInMicron);
					item.setHistogram(h);

					finishTime = System.nanoTime();
					System.err.println("Elapsed time (histogram): "
							+ (finishTime - startTime) / 1000000);
					startTime = System.nanoTime();
					...
					finishTime = System.nanoTime();
					System.err.println("Elapsed time (robust mean): "
							+ (finishTime - startTime) / 1000000);
					startTime = System.nanoTime();
	...
	/**
	 * For the user it's easier to work with a high threshold (that is, only
	 * some parts of the cells are marked red) This means we have to take into
	 * account the pixels within the threshold range
	 * 
	 * @param green
	 * @param red
	 * @param pixelToMicronFactor
	 *            conversion factor (from pixel to micron)
	 * @return distances in microns
	 * @throws OutOfMemoryError
	 * @author CO Sanchez Sorzano
	 */
	private double[] measure(ImagePlus green, ImagePlus red,
			double pixelToMicronFactor, boolean fast) throws OutOfMemoryError,
			NoSuchElementException {
		double progress = 0.0;
		...
		for (int z = 1; z <= lastZ; z++) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					// use getPixel since ImageJ stores pixels in bytes from
					// -128 to 128 (see ByteProcessor)
					// hence accessing directly would hide values greater than
					// 127 as negatives
					int pixel = green.getProcessor().getPixel(x, y); // greenPixels[y
																		// *
																		// width
																		// + x]
																		// ;
					...
					pixel = red.getProcessor().getPixel(x, y); // redPixels[y *
																// width + x] ;
		...
		System.err.println("Maximum - green: " + maxGreen + ", red: " + maxRed
				+ ". Minimum - green: " + minGreen + ", red: " + minRed);
		...
		if (errorTag != null)
			throw new NoSuchElementException("Zero " + errorTag
					+ " points found");
		...
		for (int i = 0; i < redNeeded; i++) {
			...
			for (int j = 0; j < greenNeeded; j++) {
				...
				distanceInMicron[last] = Math.sqrt(dx * dx + dy * dy + dz * dz)
						* pixelToMicronFactor;
	...
	private void drawROIs(ImagePlus imp) {
		Object rois[] = chckbxShowAll.isSelected() ? listModel.toArray()
				: regionList.getSelectedValues();
		int indexes[] = chckbxShowAll.isSelected() ? null : regionList
				.getSelectedIndices();
	...
	private static void drawRoiLabel(String label, ImageCanvas canvas,
			Rectangle r) {
		Graphics g = canvas.getGraphics();
	...
	private void importLif() {
		...
		// order to use our own filedialog (instead of awt)
